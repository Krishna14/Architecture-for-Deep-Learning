...
class Memory(Construct):
    ...
    def load8(self, inputs, outputs, arguments):
    	print "memory.py<-load8: inputs="+"self:"+str(self)+","+"inputs:"+str(inputs)+","+"outputs:"+str(outputs)+","+"arguments:"+str(arguments)+","
    ...
    def load16(self, inputs, outputs, arguments):
    	print "memory.py<-load16: inputs="+"self:"+str(self)+","+"inputs:"+str(inputs)+","+"outputs:"+str(outputs)+","+"arguments:"+str(arguments)+","
        # print 'load16 {} {}'.format(outputs, arguments)
        assert os.path.exists(self.get_cid() + '_hex'), 'file for memory missing'
        mem_fd = os.open(self.get_cid() + '_hex', os.O_RDWR)
        os.lseek(mem_fd, arguments[0], 0)
        val = np.array(array.array('h', os.read(mem_fd, arguments[1] * arguments[2] * 2)), dtype=np.int16)
        self.push_output(outputs[0], val.reshape(arguments[1], arguments[2]))

    def store8(self, inputs, outputs, arguments):
    	print "memory.py<-store8: inputs="+"self:"+str(self)+","+"inputs:"+str(inputs)+","+"outputs:"+str(outputs)+","+"arguments:"+str(arguments)+","
    ...
    def store16(self, inputs, outputs, arguments):
    	print "memory.py<-store16: inputs="+"self:"+str(self)+","+"inputs:"+str(inputs)+","+"outputs:"+str(outputs)+","+"arguments:"+str(arguments)+","
    ...
    def __init__(self, name, store_time, load_time):
    	print "memory.py<-__init__: inputs="+"self:"+str(self)+","+"name:"+str(name)+","+"store_time:"+str(store_time)+","+"load_time:"+str(load_time)+","
