...
class ProcessingElement(Construct):
    ...
    def matmul8(self, inputs, outputs, arguments):
    	print "pe.py<-matmul8: inputs="+"self:"+str(self)+","+"inputs:"+str(inputs)+","+"outputs:"+str(outputs)+","+"arguments:"+str(arguments)+","
    ...
    def matmul16(self, inputs, outputs, arguments):
    	print "pe.py<-matmul16: inputs="+"self:"+str(self)+","+"inputs:"+str(inputs)+","+"outputs:"+str(outputs)+","+"arguments:"+str(arguments)+","
        #print 'matmul {} {} {}'.format(outputs, inputs, arguments)
        ifmap = self.pop_input(inputs[0]).astype(np.int32)
        kernel = self.pop_input(inputs[1]).astype(np.int32)
        assert ifmap.shape[1] == kernel.shape[1], 'shape error for matmul'
        ofmap = np.matmul(ifmap, kernel.T)
        assert ifmap.shape[0] == ofmap.shape[0], 'output not correct shape'
        assert kernel.shape[0] == ofmap.shape[1], 'output not correct shape'
    ...
    def add8(self, inputs, outputs, arguments):
    	print "pe.py<-add8: inputs="+"self:"+str(self)+","+"inputs:"+str(inputs)+","+"outputs:"+str(outputs)+","+"arguments:"+str(arguments)+","
    ...
    def __init__(self, name, matmul8_time, add8_time):
    	print "pe.py<-__init__: inputs="+"self:"+str(self)+","+"name:"+str(name)+","+"matmul8_time:"+str(matmul8_time)+","+"add8_time:"+str(add8_time)+","
        super(ProcessingElement, self).__init__(name)
        
        #                command,   time,         function,     inputs, outputs, arguments 
        self.set_command('matmul16', matmul8_time, self.matmul16, [1, 2], [0], [3, 4, 5])
